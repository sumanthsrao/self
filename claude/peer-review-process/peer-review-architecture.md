...
Scope: Governs when and how an external independent review (ChatGPT) is used to challenge a ##Master decision, and how that review is reconciled and traced.
...
| Role | Responsibility |
|---|---|
| **##Master** | Orchestrates the SDLC, decides whether peer review is required, constructs the review request, and is the sole actor that resumes work after a review — but only once a human resolution is recorded. ##Master never treats ChatGPT's output as binding on its own. |
...
##MASTER
  ...
  CHATGPT  ──────────────────  independent review (blind or challenge mode)
   ...
   GITHUB  ──────────────────  Final Resolution section populated,
     ...
     ▼
##MASTER  ─────────────  snapshots issue into docs/10-traceability/peer-reviews/,
